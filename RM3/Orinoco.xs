...
void main(void)
{
   ...
   if (rmAllocateSubCivs(4) == true)
   {
		...
		{ //this is a mixed range and melee map
			...
			if(rmRandFloat(0,1) > 0.50)
			{ //Zapotec or Maya
				string meleeCiv = "zCPForwardCommandCenter"; //zapotec
			}
			else
			{
				meleeCiv = "Maya"; //Maya
		...
		{ //this is a all ranged or all melee map
			...
			{ //Melee only (Zapotec & Maya)
				...
				{ //Zapotec Top Left and Bottom Right
					subCiv0=rmGetCivID("zCPForwardCommandCenter");
					if (subCiv0 >= 0)
						rmSetSubCiv(0, "zCPForwardCommandCenter");
					...
					subCiv3=rmGetCivID("zCPForwardCommandCenter");
					if (subCiv3 >= 0)
						rmSetSubCiv(3, "zCPForwardCommandCenter");
				}
				else
				{ //Zapotec Bottom Left and Top Right
					...
					if (subCiv0 >= 0)
						rmSetSubCiv(0, "Maya");
					subCiv1=rmGetCivID("zCPForwardCommandCenter");
					if (subCiv1 >= 0)
						rmSetSubCiv(1, "zCPForwardCommandCenter");

					subCiv2=rmGetCivID("zCPForwardCommandCenter");
					if (subCiv2 >= 0)
						rmSetSubCiv(2, "zCPForwardCommandCenter");
   ...
   // Picks the map size
	...
	rmSetBaseTerrainMix("zFelucia_jungle");
	rmTerrainInitialize("Felucia\Felucia_Dirt50Grass", 4);
	rmSetMapType("amazonia");
	rmSetMapType("tropical");
	rmSetMapType("land");
	rmSetLightingSet("hc_revolution");
   ...
   // Build a north area
	...
	rmSetAreaMix(eastIslandID, "zFelucia_jungle");
   ...
   rmSetStatusText("",0.30);
	...
	// Middle River
	int riverID = rmRiverCreate(-1, "zFelucia River", 5, 15, 6, 9);
   ...
   for(i=1; <cNumberPlayers)
   {
	...
	else if (subCiv0 == rmGetCivID("zCPForwardCommandCenter"))
	{  
		topleftVillageID = rmCreateGrouping("Zapotec village Top Left", "cp forward command post "+rmRandInt(1,3));
	}
	else
	{  
	...
	else if (subCiv1 == rmGetCivID("zCPForwardCommandCenter"))
	{  
		bottomleftVillageID = rmCreateGrouping("Zapotec village Bottom Left", "cp forward command post "+rmRandInt(1,3));
	}
	else
	{  
	...
	else if (subCiv2 == rmGetCivID("zCPForwardCommandCenter"))
	{  
		topRightVillageID = rmCreateGrouping("Zapotec village Top Right", "cp forward command post "+rmRandInt(1,3));
	}
	else
	{  
	...
	else if (subCiv3 == rmGetCivID("zCPForwardCommandCenter"))
	{  
		bottomRightVillageID = rmCreateGrouping("Zapotec village Bottom Right", "cp forward command post "+rmRandInt(1,3));
	}
	else
	{  
    ...
    rmAddObjectDefItem(playerTreeID, "zTreeFeluciaPitcherPlant", rmRandInt(5,10), 8.0);
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaSize(forestID, rmAreaTilesToFraction(190), rmAreaTilesToFraction(280));
			rmSetAreaForestType(forestID, "zFelucia Bulbplants");
			rmSetAreaForestDensity(forestID, 0.6);
			rmSetAreaForestClumpiness(forestID, 0.4);		
			rmSetAreaForestUnderbrush(forestID, 0.6);
			rmSetAreaMinBlobs(forestID, 10);
			rmSetAreaMaxBlobs(forestID, 20);						
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaForestType(forestEastID, "zFelucia Bulbplants");
			...
			rmSetAreaMinBlobs(forestEastID, 8);
			rmSetAreaMaxBlobs(forestEastID, 20);						
			rmSetAreaMinBlobDistance(forestEastID, 10.0);
			rmSetAreaMaxBlobDistance(forestEastID, 25.0);
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaSize(forestRandomID, rmAreaTilesToFraction(80), rmAreaTilesToFraction(120));
			rmSetAreaForestType(forestRandomID, "zFelucia Bulbplants");
			...
			rmSetAreaMinBlobs(forestRandomID, 4);
			rmSetAreaMaxBlobs(forestRandomID, 10);						
			rmSetAreaMinBlobDistance(forestRandomID, 10.0);
			rmSetAreaMaxBlobDistance(forestRandomID, 25.0);
  ...
  if(rmGetIsKOTH()) {
    ...
    ypKingsHillLandfill(xLoc, xLoc, .0075, 4.5, "zFelucia_grassdirt", 0);
